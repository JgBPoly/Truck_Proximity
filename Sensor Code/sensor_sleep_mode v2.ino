...
unsigned long t_meas_sent = 0;
unsigned long DT_MEAS_SENT = 500; //ms, time between sending two measurements
...
void OnDataRecv(const uint8_t * mac, const uint8_t *incomingData, int len) {
  ...
  if (currentMillis >= t_meas_sent + DT_MEAS_SENT)  // Sensor will reply only if last meas expired after DT_MEAS_SENT ms
  {
    ...
    sleep_mode_state = false; // Device is not sleeping
    ...
    if (result == ESP_OK) {
      ...
      t_meas_sent = millis(); // Save last time if sent with success
    }
    else {
...
void readSerial(){  // Postpone spleep mode with a serial msg (ONLY FOR TESTS)
...
void loop() {
  ...
  if (!sleep_mode_state && currentMillis > previousMillis + T_AUTO_SLEEPMODE * mS_TO_S_FACTOR) { // after 5 secs
    previousMillis = currentMillis;
    sleep_mode_state = true;
